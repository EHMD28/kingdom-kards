# General

- [ ] Add proper error handling instead of just using `unwrap()`.

# Client

- [ ] Connect to specific port.
- [x] Receive data from server.
- [x] Send data to server.

# Server

<!-- For testing, I'm probably going to test using localhost. -->
- [ ] Accept TCP connections on LAN .
- [x] Accept multiple concurrent clients.
- [ ] Handle multiple concurent clients.
- [x] Receive data from client.
- [x] Send data to client.

# Model

- [x] Add type for representing cards.
- [x] Add type for representing decks.
- [x] Add type for representing players.
- [x] Add type for representing game state.

# Game

- [ ] Handle Kings
- [ ] Handle Queens
- [ ] Handle Jacks
- [ ] Handle Aces
- [ ] Handle Numbers

# User Interface

- [ ] Handle flags for switching between UIs.

## CLI

- [ ] Print debug information to CLI.

## TUI

- [ ] Create TUI basic using Ratatui.
- [ ] Show current game state.
- [ ] Show current turn. 
- [ ] Allow player to perform actions on their turn.

## GUI

- [ ] Port TUI to web using Ratzilla?
